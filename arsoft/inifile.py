            ...
            if self.original is not None and not only_data:
                ...
                if self.key is not None:
                    ...
                    if self.value is not None:
                        ret += str(self.value)
                if self.comment and not only_data:
                    if len(ret) > 0:
...
class IniFile(object):
    ...
    def section(self, section, add_if_not_exists=False):
        section_obj = self._getSection(section)
        if section_obj is None and add_if_not_exists:
            self.m_sections.append( IniSection(self, section) )
            section_obj = self.m_sections[-1]
        return section_obj
